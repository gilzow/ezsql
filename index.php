...
Version: 4.11.15
*/
define("ELISQLREPORTS_VERSION", '4.11.15');
...
function ELISQLREPORTS_create_report() {
	global $wpdb;
	ELISQLREPORTS_display_header('Create SQL Report', array("Edit Report", "Plugin Updates", "Plugin Links", "Saved Reports"));
	$GLOBALS["ELISQLREPORTS"]["reports_array"] = get_option('ELISQLREPORTS_reports_array', array());
	if (strlen(trim($GLOBALS["ELISQLREPORTS"]["Report_SQL"]))==0) {
...
//	else echo md5(serialize($args)).'='.serialize($args);
	...
	if (isset($_POST["db_date"]) && strlen($_POST["db_date"])) {
		if (isset($opts[$_POST["db_date"]]) && is_array($opts[$_POST["db_date"]])) {
		...
		} elseif (is_file(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_dir"]).$_POST["db_date"])) {
			...
			if (isset($_POST["db_nonce"]) && wp_verify_nonce($_POST["db_nonce"], $_POST["db_date"])) {
				...
				if (substr($_POST['db_date'], -7) == '.sql.gz') {
					passthru('gunzip -c "'.trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).$_POST['db_date'].'" | '.$backup_command, $errors);
					echo "<li>Restore process executed Gzip extraction with $errors error".($errors==1?'':'s').'!</li><br>';
				} elseif (substr($_POST['db_date'], -8) == '.sql.zip') {
					$zip = new ZipArchive;
					if ($zip->open(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).$_POST['db_date']) === TRUE) {
						$zip->extractTo(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']));
						$zip->close();
					}
					if (is_file(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).substr($_POST['db_date'], 0, -4))) {
						passthru($backup_command.' -e "source '.trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).substr($_POST['db_date'], 0, -4).'"', $errors);
						if ($errors) {
							$file_sql = substr($_POST['db_date'], 0, -4);
							if ($full_sql = file_get_contents(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).$file_sql)) {
								$queries = 0;
								$errors = array();
								$startpos = 0;
								...
								echo 'Error Reading File:'.trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).$file_sql;
						} else
					...
					passthru($backup_command.' -e "source '.trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).$_POST['db_date'].'"', $errors);
	...
	} elseif (isset($_GET['delete']) && is_file($delete = trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]['backup_dir']).str_replace('/', '', str_replace('\\', '', $_GET['delete']))))
		@unlink($delete);
...
function ELISQLREPORTS_menu() {
	...
	ELISQLREPORTS_set_backupdir();
	if (current_user_can("activate_plugins")) {
		if (isset($_GET["Download_SQL_Backup"]) && is_file(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_dir"]).$_GET["Download_SQL_Backup"]) && ($fp = fopen(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_dir"]).$_GET["Download_SQL_Backup"], 'rb'))) {
			header("Content-Type: application/octet-stream;");
			header('Content-Disposition: attachment; filename="'.$_GET["Download_SQL_Backup"].'"');
			header("Content-Length: ".filesize(trailingslashit($GLOBALS["ELISQLREPORTS"]["settings_array"]["backup_dir"]).$_GET["Download_SQL_Backup"]));
			fpassthru($fp);
			exit;
		}
